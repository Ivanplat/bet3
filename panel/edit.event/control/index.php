...
if (isset($_GET['delete']) && isset($_GET['id'])) {
	...
	$check_event = $db->query("SELECT * FROM events WHERE id = '?i'", $id);
	
	//проверим, есть ли событие
	if ($db->numRows($check_event) === 1) { 
	
	

	$event_db = $db->fetch($check_event);
	
	//проверим не помечено ли оно уже как удаленное
		if ($event_db['old'] != 1) {
			
			//проверим, ставили ли на это событие, если ставили, то вернем средства пользователям
			$search_event = $db->query("SELECT * FROM betting WHERE idsob = '?i'", $event_db['id']);
			
			if ($db->numRows($search_event) > 0) {
				while($return_money = $db->fetch($search_event)) {
					user_balance($return_money['how'], $return_money['who']);
				}
			}
		...
		#информируем админа
			...
			errors('Событие уже было удалено!');
		}		
