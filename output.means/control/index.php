...
if (isset($_POST['goAmount'])) {
    ...
    if ($amount && ($amount >= $settingsSystem['min_out']) && ($amount <= $settingsSystem['max_out']) ) {
        ...
        if ($insertInput) 
		{
			show('Ваш запрос на выплату принят! Перевод осуществляется в течение суток.');
			add_notification('Ваш запрос на вывод со счета <b>'.$amount.'</b> руб. принят! Ожидайте подтверждения!');
		}
        else
