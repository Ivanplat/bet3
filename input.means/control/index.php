...
if (isset($_POST['goAmount'])) {
    ...
    if ($amount && ($amount >= $settingsSystem['min_in']) && ($amount <= $settingsSystem['max_in']) ) {
        ...
        $type = (($type == 'QIWI') || ($type == 'WebMoney') || ($type == 'Visa') || ($type == 'Yandex')) ? $type : false;
        ...
        elseif ($type == 'Yandex') $purse = YANDEX;
		else $purse = Visa;
        ...
        if ($insertInput) {
			show('Переведите <b>'.$amount.'</b> руб. на кошелек платежной системы '.$type.'<br />
        <b>Номер кошелька</b>: <b style="color: #f00;">'.$purse.'</b><br>
		В комментарии к переводу укажите свой логин в системе.');
		add_notification('Ваш запрос на ввод средств в размере <b>'.$amount.'</b> руб. принят! Ожидайте подтверждения!');
		}
        else
