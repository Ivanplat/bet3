...
if (isset($_POST['captcha'])) {
    ...
    if  (!$errorCaptcha) {
        ...
        if ($login && $pas1 && $pas2 && $email) {
			...
			#записываем рефера
			$refer = isset($_SESSION['ref']) ? $_SESSION['ref'] : 0;
			
            #добавляем пользователя в БД
            $query = $db -> query("INSERT INTO users (login, password, email, balance, refer) VALUES (?s, ?s, ?s, ?i, ?s)", $login, $password, $email, $bonus_reg, $refer);
